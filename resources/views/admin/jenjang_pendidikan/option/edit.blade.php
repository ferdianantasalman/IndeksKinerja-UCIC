    ...
    <div class="container-fluid pt-4 px-4">
        <div class="col-12">
            <div class="bg-light rounded h-100 p-4">
                ...
                {{-- <a href="">{{ $id }}</a> --}}
                <form action="{{ route('jenjang_pendidikan.options.update', $options->id) }}" method="POST">
                    @csrf
                    @method('PUT')
                    <div class="mb-3">
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-control" name="question_id" id="question_id">
                            @foreach ($questions as $id => $question)
                                <option {{ $id == $options->question->id ? 'selected' : null }} value="{{ $id }}">
                                    {{ $question }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control" id="option_text" name="option_text"
                            value="{{ old('option_text', $options->option_text) }}">
                    </div>
                    <div class="mb-3">
                    ...
                    <a href="/admin/jenjang_pendidikan/options" class="btn btn-primary">Kembali</a>
